...
void GdbConnection::reply_get_library_list(Task *t) {
  ...
  if (addr == 0) {
      ...
      if (ok) {
            ...
            ntheader->Signature == IMAGE_NT_SIGNATURE)
        {
              ...
              ntheader->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
          {
            ...
            for (auto it2 = mapping_list.begin(); it2 != mapping_list.end(); it2++) {
              if (find(segments.begin(), segments.end(), it2->start()) != segments.end()) {
                if (name.empty() && !it2->fsname().empty()) {
                  name = it2->fsname();
                  break;
                }
              }
    ...
    // search remaining for regular shared objects (look here also into interpreter tables or search ELF signatures?)
    for (auto it = mapping_list.begin(); it != mapping_list.end(); it++) {
      auto km = *it;
      string name = km.fsname();
      if (!name.empty()) {
